...
static List<LooseContentRecord> ScanLooseContentInventory(List<LooseFileInventoryRecord> looseFileRecords)
{
    ...
    foreach (LooseFileInventoryRecord fileRecord in jsonFiles)
    {
        try
        {
            List<LooseContentRecord> fileContentRecords = ExtractLooseJsonContentRecords(fileRecord).ToList();
            filesParsed++;
...
static IEnumerable<LooseContentRecord> ExtractLooseJsonContentRecords(LooseFileInventoryRecord fileRecord)
{
    ...
    foreach ((JsonElement element, string jsonPath) in EnumerateJsonObjects(document.RootElement, "$"))
    {
                ...
                out string? ruleContext))
        {
            yield return new LooseContentRecord(
                ...
                DuplicatePolicy: "Report: loose content record override",
    ...
    string? overwritePath)
{
    ...
    Console.WriteLine($"Enabled loose source folders with content records: {reportRecords.Where(r => r.Mo2Enabled == true).Select(r => r.SourceFolderPath).Distinct(StringComparer.OrdinalIgnoreCase).Count()}");
    ...
    if (!options.IncludeDisabled)
        Console.WriteLine("Filter: enabled MO2 mods only for loose content analysis");
    ...
    Console.WriteLine("  Winner rule used here: same as loose files; earlier enabled modlist.txt entries override later entries, and Overwrite is highest priority.");
    ...
    Console.WriteLine();
    Console.WriteLine("Loose content record overrides:");
    ...
    Console.WriteLine($"  Reportable loose content override group(s): {duplicateGroups.Count(g => g.IsReportable)}");
    ...
    foreach (LooseContentDuplicateGroup group in duplicateGroups.Take(options.MaxDuplicateGroups))
    {
        ...
        Console.WriteLine($"CONTENT OVERRIDE: {group.IdentityKey}");
        ...
        foreach (LooseContentRecord record in orderedRecords.Take(options.MaxRecordsPerDuplicateGroup))
        {
            ...
            if (IsWinningLooseContentRecord(record, group.Records))
...
static void PrintLooseContentDuplicateOverview(List<LooseContentDuplicateGroup> duplicateGroups, int maxGroups)
{
    ...
    {
        LooseContentRecord first = group.Records.First();
        string winner = FormatLooseContentWinner(group.Records);
        Console.WriteLine($"  CONTENT    {group.ModCount,3} mod(s), {group.SourceCount,3} file(s) | {first.ContentKind} | {group.IdentityKey} | winner: {winner}");
    }
}
...
static string FormatLooseContentWinner(IEnumerable<LooseContentRecord> records)
{
    ...
    if (winner == null)
        return "no enabled loose content provider";
...
static bool IsReportableLooseContentDuplicate(LooseContentRecord record)
{
